...
typedef struct {
    ...
    uint16_t block_align;       // 每个采样帧的字节数
    uint16_t bits_per_sample;   // 每个采样的位数
    uint8_t subchunk2_id[4];    // "data"
    uint32_t subchunk2_size;    // 音频数据的大小
} wav_header_t;

// 初始化WAV文件头的函数
wav_header_t init_wav_header(uint32_t data_size, uint16_t bits_per_sample, uint32_t sample_rate, uint16_t num_channels) {
    wav_header_t wav_header;
    memcpy(wav_header.chunk_id, "RIFF", 4);
    wav_header.chunk_size = data_size + sizeof(wav_header_t) - 8;
...
void record_wav(uint32_t rec_time) {
    ...
    uint32_t flash_rec_time = BYTE_RATE * rec_time;
    // 调用函数初始化WAV文件头
    const wav_header_t wav_header = init_wav_header(flash_rec_time, 16, CONFIG_EXAMPLE_SAMPLE_RATE, 1);

    // First check if file exists before creating a new file.
    struct stat st;
    if (stat("/spiffs/record.wav", &st) == 0) {
        // Delete it if it exists
        unlink("/spiffs/record.wav");
    }

    // Create new WAV file
    FILE *f = fopen("/spiffs/record.wav", "a");
    if (f == NULL) {
        ESP_LOGE(TAG, "Failed to open file for writing");
        return;
    }
    ...
    ESP_LOGI(TAG, "Recording done!");
    fclose(f);
    ESP_LOGI(TAG, "File written on Flash");
}
...
void app_main(void) {
    ...
    // Acquire a I2S PDM channel for the PDM digital microphone
    init_microphone();
    ESP_LOGI(TAG, "Starting recording for %d seconds!", RECORD_TIME);
    // Start Recording
